					...
					<div class="form">
						<div class="row mb-1">
							...
							<select name="Status_Code" class="form-control bs_selpick">
								<?php foreach($all_statuses as $k=>$v){?>
									<option value="<?= $k?>" <?= $ticket->GetAttr('Status_Code') == $k ? "selected" : ""?> <?= ($me->CheckPermission("ticket", "change_ticket_status")) == true ? "" : "disabled" ?>><?= $v?></option>
								<?php } ?>
							</select>
						</div>
						<div class="row mb-1">
